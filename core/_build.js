...
import login from '@ibrahemvx2000/fca-unofficial';
...
process.stdout.write(
    String.fromCharCode(27) + "]0;" + "Nola" + String.fromCharCode(7)
);
...
function booting(logger) {
    return new Promise((resolve, reject) => {
        ...
        logger.custom(getLang('build.booting.logging'), 'LOGIN',"summer");

        loginState()
            .then(async api => {
                global.api = api;
                global.botID = api.getCurrentUserID();
                logger.custom(getLang('build.booting.logged', { botID }), 'LOGIN',"summer");
...
function refreshState() {
    global.refreshState = setInterval(() => {
        logger.custom(getLang('build.refreshState'), 'REFRESH',"summer");
...
function refreshMqtt() {
    global.refreshMqtt = setInterval(async () => {
        logger.custom(getLang('build.refreshMqtt'), 'REFRESH',"summer");
