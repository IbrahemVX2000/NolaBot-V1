...
async function checkAppstate(APPSTATE_PATH, APPSTATE_PROTECTION) {
    ...
    if (!isExists(APPSTATE_PATH, "file")) {
        ...
        logger.custom(getLang("modules.checkAppstate.foundAppstate"), "LOGIN","summer");
    }
    ...
    if (APPSTATE_PROTECTION !== true) {
        logger.custom(getLang("modules.checkAppstate.noProtection"), "LOGIN","summer");
...
async function getAppStateNoProtection(
    ...
    try {
        if (isJSON(appState)) {
            ...
            logger.custom(getLang("modules.checkAppstate.decrypting"), "LOGIN","summer");
