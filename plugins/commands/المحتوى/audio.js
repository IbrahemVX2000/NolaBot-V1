...
const config = {
    ...
    aliases: ["اغنية"],
    version: "1.0.3",
    description: "قم بتحميل الاصوات من يوتيوب",
    usage: 'الككلمة المفتاحية او رابط الفديو',
    cooldown: 10,
    extra: {
        "MAX_SONGS": 6
    }
...
async function getVideoInfo(id) {
    try {
        const { data } = await global.GET(`${global.nola_api.main}/ytvideodetails?id=${id}`)
...
async function searchByKeyword(keyword, MAX_SONGS) {
    try {
        if (!keyword) return [];
        const { data } = await global.GET(`${global.nola_api.main}/ytsearch?keyword=${encodeURIComponent(keyword)}&maxResults=${MAX_SONGS}`);
