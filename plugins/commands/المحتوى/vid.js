...
const config = {
    ...
    aliases: ['vid', 'يوتيوب'],
    version: "1.0.3",
    description: "حمل فديو من اليوتيوب",
    usage: 'رابط الفديو او كلمات البحث',
    cooldown: 10,
    extra: {
        "MAX_VIDEOS": 6
    }
...
async function searchByKeyword(keyword, MAX_VIDEOS) {
    try {
        if (!keyword) return [];
        const { data } = await global.GET(`${global.nola_api.main}/ytsearch?keyword=${encodeURIComponent(keyword)}&maxResults=${MAX_VIDEOS}`);
