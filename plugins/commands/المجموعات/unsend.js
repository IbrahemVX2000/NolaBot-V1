const config = {
    name: "امسحي",
    aliases: ["مسح","مسحي","حذف","احذف","احذفي"],
    description: "",
    usage: "[رد/تشغيل/ايقاف]",
    cooldown: 3,
    permissions: [0, 1, 2],
}
...
const langData = {
    ...
    "ar_SY": {
        ...
        "notReply": "يجب عليك الرد على رسالة لحذفها",
        "notBotMessage": "الرسالة التي ترد عليها ليست لي",
        "notAllowed": "هذه المجموعة غير مسموح لها بإلغاء إرسال الرسائل",
        "alreadyOn": "ميزة رسالة إلغاء إرسال البوت قيد التشغيل بالفعل",
        "on": "تم تشغيل ميزة إلغاء إرسال رسالة البوت",
        "alreadyOff": "ميزة رسالة إلغاء إرسال البوت متوقفة بالفعل",
        "off": "تم إيقاف تشغيل ميزة إلغاء إرسال رسالة البوت",
...
async function onCall({ message, args, getLang, data, userPermissions }) {
    try {
        ...
        const isInputQuery = input == "تشغيل" || input == "ايقاف";

        const isGroupAdmin = userPermissions.some(e => e == 1 || e === 2 || e === 3);
        ...
        if (isGroupAdmin && isInputQuery) {
            ...
            if (input == "تشغيل") return message.reply(getLang("notAllowedToTurnOn"));
